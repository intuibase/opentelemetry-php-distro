...
void Logger::setLogFeatures(std::unordered_map<opentelemetry::php::LogFeature, LogLevel> features) {
...
void Logger::printf(LogLevel level, const char *format, ...) const {
    ...
    auto msg = opentelemetry::utils::stringVPrintf(format, args);
    va_end(args);

    std::string output = "[OTEL] "s;
...
std::string Logger::getFormattedTime() const {
    ...
    return opentelemetry::utils::stringPrintf("[%4.4d-%2.2d-%2.2d %2.2d:%2.2d:%2.2d.%6.6d UTC]", buf.tm_year + 1900, buf.tm_mon + 1, buf.tm_mday, buf.tm_hour, buf.tm_min, buf.tm_sec, mircros);
}
...
void LoggerSinkSysLog::writeLog(std::string const &formattedOutput, std::string_view message, std::string_view time, std::string_view level, std::string_view process) const {
    // mt-safe (multithread)
    ::syslog(LOG_ALERT, PRsv, PRsvArg(formattedOutput));
}
