...
use OpenTelemetry\DistroTests\Util\Optional;
use Override;
use PHPUnit\Framework\Assert;

final class StringExpectations implements ExpectationsInterface
{
    ...
    public function assertMatches(string $actual): void
    {
        ...
        if ($this->isExpectedValueRegex) {
            Assert::assertMatchesRegularExpression($this->expectedValue->getValue(), $actual);
        } else {
    ...
    {
        $this->assertMatches(AssertEx::isString($actual));
    }
}
